...
import it.polito.tdp.nyc.model.Adiacente;
...
public class FXMLController {
    ...
    private TableColumn<Adiacente, String> clQuartiere; // Value injected by FXMLLoader
 
    @FXML // fx:id="clDistanza"
    private TableColumn<Adiacente, Double> clDistanza; // Value injected by FXMLLoader
    
    @FXML // fx:id="tblQuartieri"
    private TableView<Adiacente> tblQuartieri; // Value injected by FXMLLoader
    ...
    void doQuartieriAdiacenti(ActionEvent event) {
    	...
    	String citta=this.cmbQuartiere.getValue();
    	if(citta==null)
    	{
    		txtResult.appendText("Inserisci una citta");
    		return;
    		
    	}
    	this.tblQuartieri.setItems(FXCollections.observableArrayList(this.model.getConnessi(citta)));
    ...
    void initialize() {
      ...
      this.clQuartiere.setCellValueFactory(new PropertyValueFactory<Adiacente, String>("citta"));
        this.clDistanza.setCellValueFactory(new PropertyValueFactory<Adiacente, Double>("peso"));
    }
